...
find_subdomains() {
    ...
    find "${base_dir}" -name "*.txt" -exec cat {} + | sort -u | grep -i "${domain}" | tee "${base_dir}/all_subdomains.txt"
    echo "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"

    # Check for live domains using httprobe or httpx
    echo "Checking for live domains..."
    if command -v httprobe &> /dev/null; then
        cat "${base_dir}/all_subdomains.txt" | httprobe > "${base_dir}/live_subdomains.txt"
    elif command -v httpx &> /dev/null; then
        cat "${base_dir}/all_subdomains.txt" | httpx -silent -o "${base_dir}/live_subdomains.txt"
    else
